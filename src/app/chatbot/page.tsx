...
export default function ChatPage() {
  ...
  const { message, setMessage, handleSendMessage, handleSuggestionClick, handleKeyPress } =
    useChatLogic({
      showWelcome,
      setShowWelcome,
      ...
      >
        <TransitionWrapper showWelcome={showWelcome}>
          {showWelcome ? (
            ...
            <ChatInterface chatHistory={chatHistory} isTyping={isTyping} />
          )}
        </TransitionWrapper>
      </ResponsiveLayout>

      {/* Chat Input - Only show when not in welcome mode */}
      {!showWelcome && (
        <FloatingChatInput
          message={message}
          setMessage={setMessage}
